# -*- coding: utf-8 -*-
"""
Created on Sun Mar  7 15:51:49 2021
...
lables = np.array([0, 1]).reshape(-1,1)

loss = loss_object(labels, predictions)
loss_manual = -1*(labels*np.log(predictions) + (1-labels)*np.log(1-predictions))
loss_manual_mean = np.mean(loss_manual)

print(loss.numpy())
print(loss_manual)
print(loss_manual_mean)
...
#%%
# Sparse Categorical (not one hot encoding)
...
predictions = np.array( [ [0.2, 0.1, 0.7], [0.4, 0.3, 0.3], [0.1, 0.8, 0.1] ] )
labels = np.array( [ [0, 0, 1], [1, 0, 0], [0, 1, 0] ] )
