...
def metric_visualizer(losses_accs, save_path):
    ...
    epoch_ticks = np.linspace(1, len(losses_accs['train_losses']), 10).astype(int)
    
    ax.tick_params(labelsize=20, color='tab:blue')
    ...
    for spine_loc, spine in ax2.spines.items():
        if spine_loc in ['right', 'top']: spine.set_visible(False)
        if spine_loc in ['left']: spine.set_color('tab:orange')
    ...
    ax.set_ylabel('Cross Entropy Loss', fontsize=25, color='tab:blue')
